          ...
          <div className="bg-white rounded-2xl border border-slate-100 divide-y divide-slate-50">
            {bookings.map((booking) => {
              ...
              return (
                <div
                  key={booking.id}
                  className="relative flex items-center gap-5 px-5 py-4 hover:bg-slate-50/50 transition-colors"
                >
                  {/* Content dimmed for past rows — opacity on children, not the container,
                      so the hover highlight on the container stays at full strength */}
                  <div className={cn("flex items-center gap-5 flex-1 min-w-0", past && "opacity-50")}>
                  ...
                  <div className="relative z-10 flex items-center gap-1.5 shrink-0">
                    ...
                    {(booking.status === "PENDING" || booking.status === "CONFIRMED") && !past && (
                      <button
                        onClick={() => setConfirmCancelId(booking.id)}
                        disabled={isLoading}
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-red-50 text-red-500 hover:bg-red-100 transition-colors disabled:opacity-50"
                      >
                  ...
                  {/* Link last in DOM: opacity-50 creates a stacking context that would
                      sit above an earlier-positioned Link. Placing it last ensures it
                      wins in z-order while z-index:10 buttons still beat it. */}
                  <Link
